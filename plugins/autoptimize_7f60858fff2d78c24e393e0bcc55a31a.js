...
!function (factory) {
  ...
  $.mask = {definitions: {"_": "[0-9]",}, autoclear: !0, dataName: "mask", placeholder: "_"}, $.fn.extend({
  ...
  $(document).ready(function () {
    ...
    $fields.each(function () {
      var $this = $(this), mask = $this.data('mask');
      if (!mask) {
        return;
      }
      $this.mask(mask, {'autoclear': getOption($this.data('autoclear')),});
...
!function (e) {
  ...
  var o = {textarea: !0, input: !0, select: !0, button: !0},
    ...
    a = {move: "touchmove", cancel: "touchend", end: "touchend"}, u = /\s+/, c = {bubbles: !0, cancelable: !0},
  ...
  function m(e, t, n, o, i) {
    t = t.split(u);
    var a, c = d(e), r = t.length;

    function m(e) {
      n(e, o)
    }

    for (; r--;) (c[a = t[r]] || (c[a] = [])).push([n, m]), e.addEventListener(a, m)
  }

  function f(e, t, n, o) {
    t = t.split(u);
    var i, a, c, r = d(e), m = t.length;
    if (r) for (; m--;) if (a = r[i = t[m]]) for (c = a.length; c--;) a[c][0] === n && (e.removeEventListener(i, a[c][1]), a.splice(c, 1))
  }
  ...
  function b(e, n, o, i) {
    var a = o.pageX - n.pageX, u = o.pageY - n.pageY;
    a * a + u * u < t * t || function (e, t, n, o, i, a) {
      var u = e.targetTouches, c = e.timeStamp - t.timeStamp, r = {
        altKey: e.altKey,
        ctrlKey: e.ctrlKey,
        shiftKey: e.shiftKey,
        startX: t.pageX,
  ...
  function E(e, t) {
    var n = t.target, o = t.event, a = t.timer;
  ...
  if (m(document, "mousedown", function (e) {
    ...
    if (!e.defaultPrevented && e.moveEnabled) {
      ...
      void 0 === e.identifier ? (m(e.target, "click", p), m(document, i.move, T, n), m(document, i.end, E, n)) : (n.activeTouchmove = function (e, t) {
        !function (e, t) {
          var n = t.event, o = t.timer, i = h(e, n);
...
!function (t) {
  t.fn.twentytwenty = function (e) {
    ...
    return this.each(function () {
      var n = e.default_offset_pct, a = t(this), i = e.orientation, o = "vertical" === i ? "down" : "left",
      ...
      var c = a.find("img:first"), l = a.find("img:last");
      ...
      var w = function (t) {
        ...
        d.css("vertical" === i ? "top" : "left", "vertical" === i ? s.ch : s.cw), function (t) {
      ...
      var v = 0, p = 0, y = 0, h = 0, u = function (t) {
...
$(document).ready(function () {
  var xxl = 1579, xl = 1199, lg = 991, md = 767, sm = 575, xs = 479, ww = window.innerWidth;

  function headerSmall() {
    if ($(window).scrollTop() > 0) $('.header__row').addClass('small'); else $('.header__row').removeClass('small');
  }

  function reInitHeader() {
    var ww = window.innerWidth, servicesContact = $('.header__services_contact'),
      scrollbar = $('.header__services_list .scrollbar-inner'), servicesLi = $('.header__services_list > li'),
      menuLi = $('.header__menu > li');
    if ($('.header').hasClass('full')) {
      if (ww > xl) {
        $('.header__nav').removeClass('open');
        $('.header__icon.icon-burger_menu').removeClass('active icon-close');
        $('.header__services_link').addClass('active').find('.icon-burger_menu').addClass('icon-close');
        $('.header__services_menu').addClass('open');
      } else {
        $('.header__services_link').removeClass('active').find('.icon-burger_menu').removeClass('icon-close');
        $('.header__services_menu').removeClass('open');
        $('.header__nav').addClass('open');
        $('.header__icon.icon-burger_menu').addClass('active icon-close');
      }
    }
    if (ww > md) {
      if (servicesContact.hasClass('done')) servicesContact.removeClass('done').appendTo('.header__services_row');
      if (scrollbar.hasClass('done')) scrollbar.removeClass('done').scrollbar();
    } else {
      if (!servicesContact.hasClass('done')) servicesContact.addClass('done').appendTo('.header__nav');
      if (!scrollbar.hasClass('done')) scrollbar.addClass('done').scrollbar('destroy');
    }
    servicesLi.add(menuLi).find('ul').each(function (index, el) {
      var thisParentItem = $(this).parents('li'), thisParentItemIcon = thisParentItem.find('.icon-arrow-down'),
        addIcon = $('<span class="icon-arrow-down"></span>');
      if (ww > md) {
        if (thisParentItemIcon.hasClass('done')) thisParentItemIcon.removeClass('done').remove();
      } else {
        if (!thisParentItemIcon.hasClass('done')) addIcon.addClass('done').prependTo(thisParentItem);
      }
    });
  }

  function countSlides(currentSlide, slick, controls) {
    var current = (currentSlide ? currentSlide : 0) + 1, current = current < 10 ? '0' + current : current,
      count = slick.slideCount < 10 ? '0' + slick.slideCount : slick.slideCount;
    controls.find('.controls__current').text(current);
    controls.find('.controls__count').text(count);
  }

  function addScroll() {
    var ww = window.innerWidth, tabs = $('.tabs'), tabsTitle = $('.tabs__title'), anchors = $('.anchors'),
      anchorsList = $('.anchors__list');
    if (ww > lg) {
      if (tabsTitle.add(anchorsList).parent().is('.scrollbar-inner')) {
        tabs.add(anchors).find('.scrollbar-inner').scrollbar('destroy');
        tabsTitle.add(anchorsList).unwrap();
      }
    } else {
      if (!tabsTitle.add(anchorsList).parent().is('.scrollbar-inner')) {
        tabsTitle.add(anchorsList).wrap('<div class="scrollbar-inner"></div>');
        tabs.add(anchors).find('.scrollbar-inner').scrollbar();
      }
    }
  }
  ...
  function reInitVideo() {
    ...
    $('.video').each(function (index, el) {
      var videoInfo = $(this).find('.video__info');
      if (ww > md) {
        if (videoInfo.hasClass('done')) videoInfo.removeClass('done').appendTo($(this).find('.video__link'));
      } else {
        if (!videoInfo.hasClass('done')) videoInfo.addClass('done').appendTo($(this));
      }
    });
  }

  function reInitSpecialist() {
    var ww = window.innerWidth, specialistPosition = $('.main_specialist__position');
    if (ww > md) {
      if (specialistPosition.parent().is('.container')) {
        specialistPosition.unwrap().appendTo('.main_specialist__info');
      }
    } else {
      if (!specialistPosition.parent().is('.container')) {
        specialistPosition.appendTo('.main_specialist').wrap('<div class="container"></div>');
      }
    }
  }

  function anchorsFixed() {
    if ($('.main_service').length > 0 && $('.anchors').length > 0) {
      var windowScrollTop = $(window).scrollTop(), headerHeight = $('.header').outerHeight(),
        mainServiceHeight = $('.main_service').outerHeight(), anchorsHeight = $('.anchors').outerHeight();
      if (windowScrollTop > mainServiceHeight - headerHeight) {
        $('.anchors').addClass('fixed').css('top', headerHeight);
        $('.main_service').css('marginBottom', anchorsHeight);
        $('.header__row.small').addClass('no_shadow');
      } else {
        $('.anchors').removeClass('fixed').css('top', 0);
        $('.main_service').css('marginBottom', 0);
        $('.header__row.small').removeClass('no_shadow');
      }
    }
  }

  function onScroll() {
    var scroll_top = $(window).scrollTop(), menu_selector = '.anchors__list',
      menu = $('.header').outerHeight() + $('.anchors').outerHeight() + 50;
    $(menu_selector + ' li').each(function () {
      var hash = $(this).attr('data-anchors-id');
      if (hash != '#') {
        var target = $(hash);
        if (target.length > 0) {
  ...
  function tabHash() {
    var hash = $('[data-tab-id="' + window.location.hash + '"]');
    if ($(hash).parents().is('.tabs__title')) {
      var hashIndex = hash.index();
      $(hash).addClass('active').siblings().removeClass('active');
      $(hash).parents('.tabs').find('.tabs__body > li').hide().eq(hashIndex).fadeIn(300);
      $(window).trigger('resize.twentytwenty');
    }
  }

  function reInitFooter() {
    var ww = window.innerWidth, footerPayment = $('.footer__payment');
    if (ww > md) {
      if (footerPayment.hasClass('done')) footerPayment.removeClass('done').appendTo('.footer__logo');
    } else {
      if (!footerPayment.hasClass('done')) footerPayment.addClass('done').insertBefore('.footer__bottom');
    }
  }

  $('.header__services_list > li > ul').addClass('scrollbar-inner').wrap('<div class="scrollbar"></div>');
  if ($('.scrollbar-inner').length > 0) $('.scrollbar-inner').scrollbar();
  $(document).on('click', '[data-open-menu]', function (event) {
    event.preventDefault();
    var id = $(this).attr('href'), ww = window.innerWidth;
    if ($(this).hasClass('header__services_link') && ww <= xl) return; else {
      $(id).toggleClass('open');
      $('.header').toggleClass('full');
      $('.header__row').toggleClass('white');
      if (ww > xl) $(this).toggleClass('active').find('.icon-burger_menu').toggleClass('icon-close'); else $(this).toggleClass('active icon-close');
    }
  });
  $(document).on('click', '.header.full', function (event) {
    if ($(event.target).hasClass('header full')) {
      $('.header').removeClass('full');
      $('.header__row').removeClass('white');
      $('.header .open').removeClass('open');
      $('.header__services_link').removeClass('active').find('.icon-burger_menu').removeClass('icon-close');
      $('.header__icon').removeClass('active icon-close');
    }
  });
  if ('ontouchstart' in document.documentElement === true || ww <= md) {
    $(document).on('click', '.header__services_list > li > a, .header__menu > li > a', function (event) {
      var parentLi = $(this).parent();
      $('.header').find('.dblclick.active').not(parentLi).removeClass('dblclick active');
      if (parentLi.find('ul').length > 0) {
        if (!parentLi.hasClass('dblclick')) {
          event.preventDefault();
          parentLi.addClass('dblclick active');
        }
      }
    });
  } else {
    $(document).on('mouseover', '.header__services_list > li, .header__menu > li', function (event) {
      $(this).addClass('active');
    });
    $(document).on('mouseout', '.header__services_list > li, .header__menu > li', function (event) {
      $(this).removeClass('active');
    });
  }
  $(document).on('click ontouchstart', function (event) {
    if (!$(event.target).parents().is('.header__menu, .header__services_list')) {
      $('.header').find('.dblclick.active').removeClass('dblclick active');
    }
  });
  $(document).on('click', '.header__icon.icon-call', function (event) {
    var ww = window.innerWidth;
    if (ww > lg) {
      event.preventDefault();
      $('#callback').modal({fadeDuration: 250, showClose: false});
    }
  });
  headerSmall();
  $(window).on('scroll', headerSmall);
  reInitHeader();
  $(window).on('resize', reInitHeader);
  if ($('.main').length > 0) {
    $('.main__img').slick({
      rows: 0,
      speed: 1200,
    ...
    $('.main__list').on('init reInit afterChange', function (event, slick, currentSlide) {
      countSlides(currentSlide, slick, $('.main__controls'));
    });
    $('.main__list').slick({
      rows: 0,
      dots: true,
      speed: 1000,
      asNavFor: $('.main__img'),
      appendDots: $('.main__controls'),
      prevArrow: $('.main__controls').find('.controls__prev'),
      nextArrow: $('.main__controls').find('.controls__next'),
      dotsClass: 'controls__dots slick-dots flex middle',
      adaptiveHeight: true,
      responsive: [{breakpoint: lg + 1, settings: {dots: false}}]
    });
    $(window).on('resize', function () {
      $('.main__list').slick('setPosition');
    });
  }
  $(document).on('mousemove', '.mouse', function (event) {
    var y = event.pageY - $(this).offset().top, x = event.pageX - $(this).offset().left;
    $(this).find('.mouse__light').css({top: y + 'px', left: x + 'px'});
  });
  if ($('.slider').length > 0) {
    $('.slider').each(function (index, el) {
      var list = $(this).find('.slider__list'), controls = $(this).find('.slider__controls'),
        ww = window.innerWidth;
      if (list.hasClass('col_4')) var slidesToShow = 4; else if (list.hasClass('col_3')) var slidesToShow = 3; else slidesToShow = 1;
      if (list.hasClass('col_3') || list.hasClass('col_4')) {
        var slidesToShowResponsive = 2, adaptiveHeightResponsive = false, variableWidth = true;
      } else {
        var slidesToShowResponsive = 1, adaptiveHeightResponsive = true, variableWidth = false;
      }
      if (controls.find('.slider__link').length > 0) var dots = false; else var dots = true;
      list.on('init breakpoint', function (event, slick, breakpoint) {
        if (slick.options.slidesToShow >= slick.slideCount) {
          if (!controls.find('.slider__link').length > 0) {
            controls.hide();
          }
        } else controls.show();
      });
      list.slick({
        ...
        dots: true,
        appendDots: controls,
        prevArrow: controls.find('.slider__prev'),
        nextArrow: controls.find('.slider__next'),
        dotsClass: 'slider__dots slick-dots flex middle',
        ...
        responsive: [{
          ...
          settings: {
            ...
            adaptiveHeight: adaptiveHeightResponsive,
            variableWidth: false
          }
        }, {
          breakpoint: sm + 1,
  ...
  if ($('.our_equipment').length > 0) {
    $('.our_equipment__list').on('init reInit afterChange', function (event, slick, currentSlide) {
      countSlides(currentSlide, slick, $('.our_equipment__controls'));
      if (slick.slideCount <= 1) $('.our_equipment__controls').hide(); else $('.our_equipment__controls').show();
    });
    $('.our_equipment__list').slick({
      rows: 0,
      dots: true,
      speed: 700,
      appendDots: $('.our_equipment__controls'),
      prevArrow: $('.our_equipment__controls').find('.controls__prev'),
      nextArrow: $('.our_equipment__controls').find('.controls__next'),
      dotsClass: 'controls__dots slick-dots flex middle',
      responsive: [{breakpoint: lg + 1, settings: {dots: false}}, {
        breakpoint: md + 1,
        settings: {adaptiveHeight: true, dots: false}
      }]
    });
  }
  if ($('.tabs').length > 0) {
    $('.tabs').each(function (index, el) {
      var tabsTitleItem = $(this).find('.tabs__title > li'), tabsBodyItem = $(this).find('.tabs__body > li');
      tabsTitleItem.first().addClass('active');
      tabsBodyItem.first().addClass('active');
      tabsTitleItem.on('click', function () {
        var thisItem = $(this), thisIndex = thisItem.index();
        if (thisItem.hasClass('active')) return false; else {
          $(this).removeClass('active');
          tabsBodyItem.hide().eq(thisIndex).fadeIn(300);
          thisItem.addClass('active').siblings().removeClass('active');
          $(window).trigger('resize.twentytwenty');
          if (tabsBodyItem.find('.our_equipment').length > 0) $('.our_equipment__list').slick('setPosition');
          if (tabsBodyItem.find('.slider').length > 0) $('.slider__list').slick('setPosition');
        }
      });
    });
    tabHash();
    $(window).on('hashchange', tabHash);
    addScroll();
    $(window).on('resize', addScroll);
  }
  if ($('[data-gallery]').length > 0) {
    $('[data-gallery]').each(function (index, el) {
      var gallery = $(this);
      if (gallery.data('gallery') === 'slider') var counter = false; else var counter = true;
      if (gallery.data('gallery') === 'this') var selector = 'this'; else var selector = 'a';
      gallery.lightGallery({selector: selector, counter: counter});
    });
  }
  if ($('[data-twentytwenty]').length > 0) {
    $('[data-twentytwenty]').twentytwenty({before_label: 'До', after_label: 'После'});
  }
  reInitVideo();
  $(window).on('resize', reInitVideo);
  $(document).on('click', '.faq__question', function (event) {
    event.preventDefault();
    ...
    item.parent().find('.faq__answer').stop().slideUp(300);
    $(this).next().stop().slideDown(300);
  });
  $(document).on('click', '.anchors__list li', function (event) {
    event.preventDefault();
    $(window).off('scroll', onScroll);
    var id = $(this).attr('data-anchors-id'), ww = window.innerWidth, number = 0;
    $(this).addClass('active').siblings().removeClass('active');
    if (ww > lg) number = 50; else number = 30;
    if ($('.main_service').length > 0) number = number + $(this).parents('.anchors').outerHeight();
    if (id != '#') {
      if ($(id).length > 0) {
        if ($(id).parents().is('.tabs__title')) {
          $(id).parents('.tabs').find('.tabs__body > li').hide().eq($(id).index()).fadeIn(300);
          $(id).addClass('active').siblings().removeClass('active');
          $(window).trigger('resize.twentytwenty');
          if ($('.our_equipment').length > 0) $('.our_equipment__list').slick('setPosition');
          if ($('.slider').length > 0) $('.slider__list').slick('setPosition');
        }
        $('html, body').animate({scrollTop: $(id).offset().top - $('.header').outerHeight() - number}, 1000, function () {
          setTimeout(function () {
  ...
  $(window).on('load', function () {
    setTimeout(anchorsFixed, 300);
  });
  $(window).on('scroll', anchorsFixed);
  reInitSpecialist();
  $(window).on('resize', reInitSpecialist);
  reInitFooter();
  $(window).on('resize', reInitFooter);
  $(document).on('click', '[data-modal]', function (event) {
    event.preventDefault();
    var id = $(this).attr('href'), object = $(this).data('object');
    if (id == '#form1') {
      $(id).find('input[name=page_info]').val(object);
    }
    $(id).modal({fadeDuration: 250, showClose: false});
  });
  if ($('[data-countdown]').length > 0) {
    $('[data-countdown]').each(function (index, el) {
      var thisEl = $(this), dateStart = thisEl.data('date-start').replace(/\./g, '/'),
        dateEnd = thisEl.data('date-end').replace(/\./g, '/');
      thisEl.countdown(dateEnd, function (event) {
        if (event.type == 'finish') {
          if (thisEl.parent().hasClass('main_small__promotions')) {
            thisEl.html('Акция закончилась').toggleClass('main_small__finish main_small__deadline');
          } else {
            thisEl.html('Акция закончилась').toggleClass('picture__finish picture__deadline');
          }
        } else {
          thisEl.html('<span class="icon-clock"></span>' + event.strftime('до конца акции: %D дней %H часов'));
        }
      });
    });
  }
  $(document).on('wpcf7mailsent', '.wpcf7', function (event) {
    $.modal.close();
    $('#thanks').modal({fadeDuration: 250, showClose: false});
  });
  $(document).on('click', '.main_service__btn', function (event) {
    if ($('.main_service__select option:selected').val() == '') return;
    var tag = {'action': 'filtering_by_tag', 'term_id': $('.main_service__select option:selected').val()}
    $.ajax({
      type: 'POST', url: mytheme.ajax_url, data: tag, beforeSend: function (xhr) {
        $('.preload').fadeIn(300);
      ...
      $('#filter_result img').each(function (index, el) {
        $(this).on('load', function () {
          if (index === ($('#filter_result img').length - 1)) {
            $('.preload').fadeOut(300, function () {
              $('html, body').animate({scrollTop: $('#filter_result_anchor').offset().top - $('.header').outerHeight() - 30}, 500);
            });
          }
        });
      });
  ...
  $(document).on('click', '.link_more_post', function (event) {
    event.preventDefault();
    var thisEl = $(this), postContent = thisEl.parents('li').find('.post_content'),
      offset = postContent.children().length;
    $.ajax({
      type: 'POST',
      url: mytheme.ajax_url,
      data: {
        'action': 'more_post',
        'term_id': thisEl.data('terms'),
        'nonce': mytheme.field_nonce,
        'post_type': thisEl.data('post'),
        'offset': offset
      },
      dataType: 'json',
      beforeSend: function (xhr) {
        if (!thisEl.hasClass('no_load_more')) {
          $('.preload').fadeIn(300);
        }
      },
      success: function (json) {
        postContent.append(json['content']);
        if (!json['more']) {
          thisEl.addClass('no_load_more');
        }
      },
      error: function (jqXHR, textStatus, errorThrown) {
        console.log(jqXHR + " :: " + textStatus + " :: " + errorThrown);
      ...
      postContent.find('img').each(function (index, el) {
        $(this).on('load', function () {
          if (index === (postContent.find('img').length - 1)) $('.preload').fadeOut(300);
        });
      });
    });
  });
  var prices_offset = 1, faq_offset = 3;
  $(document).on('click', '.btn_load_more', function (event) {
    event.preventDefault();
    var thisEl = $(this);
    thisEl.addClass('loading');
    if (thisEl.data('field') == 'service_price_list') {
      $offset = prices_offset;
    }
    if (thisEl.data('field') == 'service_faq') {
      $offset = faq_offset;
    }
    $.post(mytheme.ajax_url, {
      'action': 'field_show_more',
      'field': thisEl.data('field'),
      'nonce': mytheme.field_nonce,
      'post_id': mytheme.post_id,
      'offset': $offset,
    }, function (json) {
      thisEl.removeClass('loading');
      if (json['field'] == 'service_price_list') {
        thisEl.before(json['content']);
        prices_offset = json['offset'];
      }
      if (json['field'] == 'service_faq') {
        thisEl.parents('.faq').find('.faq__list').append(json['content']);
        faq_offset = json['offset'];
      }
      if (!json['more']) {
        thisEl.addClass('no_load_more');
      }
  ...
  $(document).on('click', '.link_more_gallery', function (event) {
    event.preventDefault();
    var thisEl = $(this), galleryContent = thisEl.parents('li').find('.gallery_content'),
      offset = galleryContent.children().length;
    $.ajax({
      type: 'POST',
      url: mytheme.ajax_url,
      data: {
      ...
      success: function (json) {
        galleryContent.append(json['content']);
        if (!json['more']) {
          thisEl.addClass('no_load_more');
        }
      },
      ...
      galleryContent.find('img').each(function (index, el) {
        $(this).on('load', function () {
          if (index === (galleryContent.find('img').length - 1)) {
            if (galleryContent.find('[data-twentytwenty]').length > 0) {
              galleryContent.find('.before_after__img').not('.twentytwenty-container').twentytwenty({
                before_label: 'До',
                after_label: 'После'
              });
            }
            if (galleryContent[0].hasAttribute('data-gallery')) {
              galleryContent.data('lightGallery').destroy(true);
              galleryContent.lightGallery({selector: 'a', counter: true});
            }
            $('.preload').fadeOut(300);
          }
        });
  ...
  $(document).on('click', '.read_more_button a', function (event) {
    event.preventDefault();
    $(this).parent().prev('.read_more_content').slideToggle('300');
    $(this).text($(this).text() == 'Читать больше' ? 'Читать меньше' : 'Читать больше');
  });
  $(document).on('click', '.main_service__btns .link_more, .main_service__price a', function (event) {
    event.preventDefault();
    var id = $(this).attr('href');
    $('html, body').animate({scrollTop: $(id).offset().top - $('.header').outerHeight() - $('.anchors').outerHeight()}, 700);
  });
  if ($('#map').length > 0) {
    var latitude = $('#map').data('latitude'), longitude = $('#map').data('longitude');
    ymaps.ready(function () {
      var myMap = new ymaps.Map('map', {zoom: 17, controls: [], center: [+latitude, +longitude]}),
        myPlacemark = new ymaps.Placemark(myMap.getCenter(), {balloonContent: 'Наша клиника'}, {
          iconLayout: 'default#image',
          iconImageHref: mytheme.template_url + '/assets/img/contacts/placemark.svg',
          iconImageSize: [46, 60],
